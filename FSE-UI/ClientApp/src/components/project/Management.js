...
export class Management extends Component {
    ...
    getProject = (prevStateflag) => {
        fetch(`${PROJECT_SERVICE_URL}/GetAllActiveProject`)
            .then(res => {
                console.log("***********************");
    ...
    searchProject = e => {
        e.preventDefault();
        fetch(`${PROJECT_SERVICE_URL}/GetProjectByName?prjNm=${this.state.srchPrjName}`)
            .then(res => {
    ...
    submitNew = e => {
        ...
        fetch(`${PROJECT_SERVICE_URL}/AddProject`, {
            method: 'post',
            headers: {
